...
- [~] **Phase 3** — Event extractor (`agent/extractor.py`) + Corridor Risk Index v1 (`core/risk.py`). CRI verified with real O/S/X data: S spikes to 0.67-0.97 on 28 Feb-1 Mar 2026 while O is still 0 (no AIS-visible disruption yet), O only catches up from 3 Mar -- exit criterion ("S leads O") confirmed. E pending: extraction is running against real GDELT/Groq data (MVP-capped at 500 articles/corridor), not yet folded into a re-verified CRI table.
